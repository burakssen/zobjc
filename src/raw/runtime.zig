...
test "raw.runtime: class lookup and inspection" {
    const cls = objc_getClass("NSObject");
    try testing.expect(cls != null);

    const name = class_getName(cls);
    try testing.expectEqualStrings("NSObject", std.mem.span(name));

    const super_cls = class_getSuperclass(cls);
    try testing.expect(super_cls == null);

    try testing.expect(class_isMetaClass(cls) != types.YES);

    const meta_cls = objc_getMetaClass("NSObject");
    try testing.expect(meta_cls != null);
    try testing.expect(class_isMetaClass(meta_cls) == types.YES);

    const size = class_getInstanceSize(cls);
    try testing.expect(size >= @sizeOf(usize));

    const sel_init = objc.sel_registerName("init");
    try testing.expect(class_respondsToSelector(cls, sel_init) == types.YES);
}

test "raw.runtime: dynamic class creation, method/ivar addition, and disposal" {
    const super_cls = objc_getClass("NSObject");
    try testing.expect(super_cls != null);

    const unique_name = "RawTestClass_DynamicPair";
    const new_cls = objc_allocateClassPair(super_cls, unique_name, 0);
    try testing.expect(new_cls != null);

    const ivar_added = class_addIvar(
        new_cls,
        "_testIvar",
        @sizeOf(usize),
        @alignOf(usize),
    ...
    try testing.expect((ivar_added == types.YES));

    const dummy_imp: types.IMP = @ptrCast(&struct {
        fn dummy(self: types.id, op: types.SEL) callconv(.c) usize {
            _ = self;
            _ = op;
            return 42;
        }
    ...
    const sel_test = objc.sel_registerName("rawTestMethod");
    const method_added = class_addMethod(
        new_cls,
        sel_test,
        dummy_imp,
        "Q@:",
    );
    try testing.expect((method_added == types.YES));

    objc_registerClassPair(new_cls);

    const registered = objc_getClass(unique_name);
    try testing.expectEqual(new_cls, registered);

    const ivar = class_getInstanceVariable(new_cls, "_testIvar");
    try testing.expect(ivar != null);
    const ivar_name = ivar_getName(ivar);
    try testing.expect(ivar_name != null);
    try testing.expectEqualStrings("_testIvar", std.mem.span(ivar_name.?));

    const method = class_getInstanceMethod(new_cls, sel_test);
    try testing.expect(method != null);
    try testing.expectEqual(sel_test, method_getName(method));

    objc_disposeClassPair(new_cls);
}

test "raw.runtime: protocol inspection" {
    const proto = objc_getProtocol("NSObject");
    try testing.expect(proto != null);

    const name = protocol_getName(proto);
    try testing.expectEqualStrings("NSObject", std.mem.span(name));

    const cls = objc_getClass("NSObject");
    try testing.expect(cls != null);
    try testing.expect(class_conformsToProtocol(cls, proto) == types.YES);
}
...
test "raw.runtime: associated objects" {
    ...
    defer {
        ...
        const ReleaseFn = *const fn (types.id, types.SEL) callconv(.c) void;
        const release_fn: ReleaseFn = @ptrCast(&message.objc_msgSend);
    ...
    const retrieved = objc_getAssociatedObject(obj, &assoc_key);
    try testing.expectEqual(assoc_val, @intFromPtr(retrieved));

    objc_setAssociatedObject(obj, &assoc_key, null, OBJC_ASSOCIATION_ASSIGN);
    try testing.expect(objc_getAssociatedObject(obj, &assoc_key) == null);
}
