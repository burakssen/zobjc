...
test "raw.message: direct invocation of objc_msgSend" {
    ...
    const sel_class = objc.sel_registerName("class");
    const ClassFn = *const fn (types.id, types.SEL) callconv(.c) types.Class;
    const class_fn: ClassFn = @ptrCast(&objc_msgSend);
    ...
    const release_fn: ReleaseFn = @ptrCast(&objc_msgSend);
...
test "raw.message: direct invocation of objc_msgSendSuper" {
    ...
    var super: types.objc_super = .{
        .receiver = obj,
        .super_class = cls,
    };

    const SuperClassFn = *const fn (*types.objc_super, types.SEL) callconv(.c) types.Class;
    const super_class_fn: SuperClassFn = @ptrCast(&objc_msgSendSuper);
    const sel_class = objc.sel_registerName("class");
...
test "raw.message: dispatch symbol references" {
    ...
    if (comptime availability.has_msgSendStret) {
        _ = &objc_msgSend_stret;
        _ = &objc_msgSendSuper_stret;
        _ = &method_invoke_stret;
        _ = &_objc_msgForward_stret;
    }
}
