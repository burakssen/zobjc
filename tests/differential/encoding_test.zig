...
//! and verifying parser round-tripping across standard runtime encodings.
...
test "differential: parser round-trip on Clang encodings" {
    const encodings = [_][*:0]const u8{
        fixture_encode_char(),
        fixture_encode_int(),
    ...
    for (encodings) |raw_enc| {
        const span = std.mem.span(raw_enc);
        var parsed = try objc.encoding.parse(testing.allocator, span);
        defer parsed.deinit(testing.allocator);

        const rendered = try objc.encoding.encode(testing.allocator, parsed);
        defer testing.allocator.free(rendered);
        try testing.expectEqualStrings(span, rendered);
    }
}

test "differential: live runtime class method encodings parse cleanly" {
    const NSObject = objc.getClass("NSObject").?;
    const method = NSObject.instanceMethod(objc.sel("init")).?;
    const enc = method.typeEncoding().?;

    // Parse method encoding signature
    var sig = try objc.encoding.parseMethod(testing.allocator, enc);
    defer sig.deinit(testing.allocator);

    // Initializer returns an object (@) and takes self (@) and _cmd (:)
